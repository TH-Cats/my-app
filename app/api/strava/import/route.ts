...
async function importForAthlete(athleteId: string, limit: number) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return importForAthlete(String(athleteId), Number(limit ?? 30));
  } catch (e) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return importForAthlete(athleteId, limit);
  } catch (e) {
    return Response.json({ error: 'Internal server error' }, { status: 500 });
  }
}
